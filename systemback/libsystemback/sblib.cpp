...
bool sb::islnxfs(cQStr &path)
{
    ...
    return file.open() && file.setPermissions(QFile::ReadOwner | QFile::WriteOwner | QFile::ExeOwner | QFile::ReadGroup | QFile::ExeGroup | QFile::ReadOther) && (file.permissions() & (QFile::ReadOwner | QFile::WriteOwner | QFile::ExeOwner | QFile::ReadGroup | QFile::ExeGroup | QFile::ReadOther));
}
...
uchar sb::fcomp(cQStr &file1, cQStr &file2)
{
    ...
    return stat(bstr(file1), &fstat[0]) || stat(bstr(file2), &fstat[1]) ? 0 : fstat[0].st_size == fstat[1].st_size && fstat[0].st_mtim.tv_sec == fstat[1].st_mtim.tv_sec ? fstat[0].st_mode == fstat[1].st_mode && fstat[0].st_uid == fstat[1].st_uid && fstat[0].st_gid == fstat[1].st_gid ? 2 : 1 : 0;
}
...
bool sb::cpertime(cQStr &srcitem, cQStr &newitem, bool skel)
{
    ...
    else if(istat[0].st_mode != istat[1].st_mode && chmod(nitem, istat[0].st_mode)) return err();
...
bool sb::recrmdir(cbstr &path, bool slimit)
{
    ...
    if(dir)
    {
        auto size([](cbstr &spath) -> ullong {
                struct stat fstat;
                if(stat(spath, &fstat)) return 134217729;
        ...
        while(! ThrdKill && (ent = readdir(dir)))
        {
            ...
            if(! like(iname, dd))
            {
                ...
                case DT_UNKNOWN:
                    switch(stype(fpath)) {
                    case Isfile:
                        if(slimit && size(fpath) > 134217728) continue;
                    default:
                        rmfile(fpath);
                        continue;
                    case Isdir:;
                ...
                case DT_DIR:
                    ...
                    break;
                case DT_REG:
                    if(slimit && size(fpath) > 134217728) break;
                default:
                    rmfile(fpath);
                }
            }
...
bool sb::thrdcrtrpoint(cQStr &trgt)
{
...

    {
        ...
        {
            QStr dirs[]{"./bin", "./boot", "./etc", "./lib", "./lib32", "./lib64", "./libx32", "./opt", "./sbin", "./selinux", "./srv", "./usr", "./var"};
            uint bbs[]{10000, 20000, 100000, 500000, 10000, 10000, 10000, 100000, 10000, 10000, 10000, 10000000, 1000000}, ibs[]{500, 1000, 10000, 20000, 500, 500, 500, 5000, 1000, 500, 500, 500000, 50000};
        ...
        if(isdir("./snap"))
        {
            sysitms[13].reserve(10000), sysitmst[13].reserve(500);
            if(! rodir("./snap", sysitms[13], sysitmst[13])) return out();
        }

        uint anum(0), cnum(0);
        for(uchar a(0) ; a < 13 ; ++a) anum += sysitmst[a].count();
        QSL rplst;
        QBA *cditms;
        QUCL *cditmst;
        uchar cperc;
    ...
    {
        {
            ...
            {
                QStr dirs[]{srcdir % "/bin", srcdir % "/boot", srcdir % "/etc", srcdir % "/lib", srcdir % "/lib32", srcdir % "/lib64", srcdir % "/libx32", srcdir % "/opt", srcdir % "/sbin", srcdir % "/selinux", srcdir % "/snap", srcdir % "/srv", srcdir % "/usr", srcdir % "/var"};
                uint bbs[]{10000, 20000, 100000, 500000, 10000, 10000, 10000, 100000, 10000, 10000, 10000, 10000, 10000000, 1000000}, ibs[]{500, 1000, 10000, 20000, 500, 500, 500, 5000, 1000, 500, 500, 500, 500000, 50000};
            ...
            edetect(elst);
            QSL dlst{"/bin", "/boot", "/etc", "/lib", "/lib32", "/lib64", "/libx32", "/opt", "/sbin", "/selinux", "/snap", "/srv", "/usr", "/var"}, excl[]{{"_lost+found_", "_Systemback_"}, {"_lost+found_", "_lost+found/*", "*/lost+found_", "*/lost+found/*", "_Systemback_", "_Systemback/*", "*/Systemback_", "*/Systemback/*"}};
            QStr mnts;
                        ...
                        for(cQStr &item : sdlst)
                        {
                            ...
                            if((inc || ! (item.endsWith('~') || exclcheck(elst, item))) && ! exist(srcd % '/' % item))
                            {
                                ...
                                case Isdir:
                                    ...
                                    break;
                                case Isfile:
                                    if(! inc && fsize(trgi) > 134217728) break;
                                case Islink:
                                    rmfile(trgi);
                                }
                            }
                    ...
                    while(! in.atEnd())
                    {
                        ...
                        if(! like(item, excl[0]) && ((inc = ! (ilst.isEmpty() || like(item, {"_.*", "_snap_", "_snap/*"}))) || ! exclcheck(elst, item)))
                        {
                            ...
                            case Isdir:
                                        ...
                                        for(cQStr &sitem : sdlst)
                                        {
                                            if(! (like(sitem, excl[1]) || exclcheck(elst, item % '/' % sitem) || exist(srci % '/' % sitem)))
                                            {
                                                ...
                                                case Isdir:
                                                    ...
                                                    break;
                                                case Isfile:
                                                    if(! inc && fsize(strgi) > 134217728) break;
                                                case Islink:
                                                    rmfile(strgi);
                                                }
                                            }
                                ...
                                break;
                            case Isfile:
                                skppd = ! inc && fsize(srci) > 134217728;
...
bool sb::thrdscopy(uchar mthd, cQStr &usr, cQStr &srcdir)
{
...

    {
        ...
        {
            QStr dirs[]{srcdir % "/bin", srcdir % "/boot", srcdir % "/etc", srcdir % "/lib", srcdir % "/lib32", srcdir % "/lib64", srcdir % "/libx32", srcdir % "/opt", srcdir % "/sbin", srcdir % "/selinux", srcdir % "/srv", srcdir % "/usr", srcdir % "/var"};
            uint bbs[]{10000, 20000, 100000, 500000, 10000, 10000, 10000, 100000, 10000, 10000, 10000, 10000000, 1000000}, ibs[]{500, 1000, 10000, 20000, 500, 500, 500, 5000, 1000, 500, 500, 500000, 50000};
        ...
        {
            sysitms[13].reserve(10000), sysitmst[13].reserve(500);
            if(! rodir("./snap", sysitms[13], sysitmst[13])) return false;
        }
                    ...
                    if(! (cditmst = &homeitmst[mthd == 5 ? 0 : a])->isEmpty())
                    {
                        ...
                        while(! in.atEnd())
                        {
                            ...
                            if((mthd == 5 || (! (like(item, excl) || exclcheck(elst, item)) && (macid.isEmpty() || ! item.contains(macid)))) && (! srcdir.isEmpty() || exist(srcd[1] % '/' % item)))
                            {
                                ...
                                case Isfile:
                                    QStr srci(srcd[1] % '/' % item);
                                    skppd = like(mthd, {2, 3}) && fsize(srci) > 134217728;
        ...
        QSL dlst{"/bin", "/boot", "/etc", "/lib", "/lib32", "/lib64", "/libx32", "/opt", "/sbin", "/selinux", "/srv", "/usr", "/var", "/snap"}, excl[]{{"_lost+found_", "_Systemback_"}, {"_lost+found_", "_lost+found/*", "*/lost+found_", "*/lost+found/*", "_Systemback_", "_Systemback/*", "*/Systemback_", "*/Systemback/*"}, {"_/etc/udev/rules.d*", "*-persistent-*"}};
...
bool sb::thrdlvprpr(bool iudata)
{
...

    {
        ...
        for(cQStr &item : {"/bin", "/boot", "/etc", "/lib", "/lib32", "/lib64", "/libx32", "/opt", "/sbin", "/selinux", "/usr", "/initrd.img", "/initrd.img.old", "/vmlinuz", "/vmlinuz.old"})
            if(isdir(item))
            {
                    ...
                    if(! (like(item, excl) || exclcheck(elst, item)))
                    {
                        ...
                        if(exist(srci))
                        {
                            ...
                            case Isdir:
                                ...
                                break;
                            case Isfile:
                                if(iudata || fsize(srci) <= 134217728)
                                {
                                    if(uhl)
                                    {
                                        if(! crthlnk(srci, usdir % '/' % item)) return false;
